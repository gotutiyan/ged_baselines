...
def binarised_labels(labels):
    return ['CORRECT' if l == 'CORRECT' else 'INCORRECT' for l in labels]
...
def main(args):
    ...
    # To solve this, we add a 'CORRECT' label for the truncated token.
    # The reason why we choose 'CORRECT' is that it is the majority label.
    for i in range(len(srcs)):
        diff = len(labels[i]) - len(results[i])
        results[i] += ['CORRECT'] * diff
    ...
    p = results['INCORRECT']['precision']
    r = results['INCORRECT']['recall']
    print(f'Precision: {p}\nRecall: {r}\nF0.5: {f05(p, r)}')
